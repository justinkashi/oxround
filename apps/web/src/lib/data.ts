// Data layer. Supabase is the only runtime data source.
"use client";
...
export function supabase(): SupabaseClient {
  ...
  if (!sb) sb = createBrowserClient(env.url, env.anon);
  return sb;
}
...
// The signed-in member's own gym_members row.
...
export type MemberProfileUpdate = {
  ...
  weight_class?: string;
  skill_level?: string;
  availability?: import("@/lib/types").AvailabilityWindow[];
};
...
export async function updateMember(
  ...
  const optional: Partial<Pick<GymMember, "date_of_birth" | "joined_at" | "weight_class" | "skill_level" | "availability">> = {};
  ...
  if ("skill_level" in fields) optional.skill_level = (fields.skill_level?.trim() || null) as GymMember["skill_level"];
  if ("availability" in fields) optional.availability = fields.availability?.length ? fields.availability : null;
...
export async function attendanceSummaries(): Promise<MemberAttendanceSummary[]> {
  ...
  const all = await listCheckIns(2000);
  const now = Date.now();
...
export async function bookingCounts(sessionIds: string[]): Promise<Record<string, { booked: number; waitlisted: number }>> {
  ...
  const rows = ((await supabase().from("class_bookings").select("session_id, status").in("session_id", sessionIds)).data as Array<{ session_id: string; status: BookingStatus }> | null) ?? [];
  for (const b of rows) {
