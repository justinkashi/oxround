  ...
  return (
    <div>
      ...
      <div className="mb-6 rounded-lg border border-neutral-200 bg-white p-4">
        <h2 className="mb-2 text-sm font-semibold uppercase text-neutral-500">Broadcast to all members</h2>
        <div className="flex gap-2">
          <input value={broadcast} onChange={(e) => setBroadcast(e.target.value)} placeholder="Message everyone…" className="flex-1 rounded-md border border-neutral-300 px-3 py-2 text-sm" />
          <button onClick={sendBroadcast} className="rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand-dark">Send to all</button>
        </div>
        {broadcasts.length > 0 && <div className="mt-2 text-xs text-neutral-400">Last broadcast: {broadcasts[0].body}</div>}
      </div>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-lg border border-neutral-200 bg-white">
          <div className="border-b border-neutral-100 px-4 py-2 text-xs font-semibold uppercase text-neutral-500">Conversations</div>
          {Object.keys(conversations).length === 0 && <div className="p-4 text-sm text-neutral-400">No member messages yet</div>}
          ...
          <div className="border-t border-neutral-100 p-2">
            <select value="" onChange={(e) => e.target.value && setSelected(e.target.value)} className="w-full rounded-md border border-neutral-300 px-2 py-1.5 text-sm">
              <option value="">Start a message to…</option>
        ...
        <div className="rounded-lg border border-neutral-200 bg-white md:col-span-2">
          {!selected ? (
            <div className="p-8 text-center text-sm text-neutral-400">Pick a conversation</div>
          ) : (
                  ...
                  return (
                    <div key={m.id} className={`flex ${fromMember ? "justify-start" : "justify-end"}`}>
                      <div className={`max-w-[75%] rounded-lg px-3 py-2 text-sm ${fromMember ? "bg-neutral-100" : "bg-brand text-white"}`}>
                        {m.body}
                        <div className={`mt-0.5 text-[10px] ${fromMember ? "text-neutral-400" : "text-white/70"}`}>{new Date(m.created_at).toLocaleString("en-CA", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</div>
                      </div>
                    </div>
                  );
                })}
                {thread.length === 0 && <div className="text-center text-sm text-neutral-400">No messages yet — say hi.</div>}
              </div>
              <div className="flex gap-2 border-t border-neutral-100 p-3">
                <input value={reply} onChange={(e) => setReply(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendReply()} placeholder="Reply…" className="flex-1 rounded-md border border-neutral-300 px-3 py-2 text-sm" />
                <button onClick={sendReply} className="rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand-dark">Send</button>
              </div>
            </div>
          )}
        </div>
