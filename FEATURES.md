...
- ✅/⬜ **Authentication & role routing:** ✅ real email+password login (all roles) with magic-link + forgot-password fallbacks, first-login set-password screen (skippable, `user_metadata.password_set` flag), invite-only, server-side code exchange, session guard, logout, **one login → role router (owner/manager→CRM, coach/receptionist→restricted CRM, member→/app), member/staff two-way guard, no-access page** (Step 6A/6B). ⬜ join-via-invite-code.
...
- ✅/⬜ **Communication:** ✅ announcements with types (closure/fight/event), pinning, read counts, reactions, quiet Post by default + optional Notify members checkbox that creates notification rows. ⬜ image uploads (+compression), **announcement expiry/auto-archive** **[added]**, **email blast to members** **[added]**, deactivation notification (member is told their QR stopped working — never surprised at the door).
