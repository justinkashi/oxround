...
- **TO DEPLOY:** (1) `git push` — Vercel builds the new UI. (2) **Migrations 0009 + 0010 NOT fully live** (Supabase connector kept dropping): only 0009's function-hardening chunk applied. Paste `supabase/migrations/00000000000009_db_hardening.sql` then `00000000000010_timeline_tasks_attachments.sql` into the Supabase dashboard SQL editor (safe to re-run: DROP IF EXISTS / IF NOT EXISTS throughout). Until 0010 runs, the Fighter Card's Timeline/Tasks/Files tabs and lead $-values will error politely on production (demo mode unaffected). (3) Still pending from last session: `supabase functions deploy invite-member` + `supabase secrets set APP_URL=<vercel url>`.
