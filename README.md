...
- **Step 6 REMAINING:** two-way messaging + community tab (need new `conversations`/`messages` tables + migration), server-side member invite email (service-role Edge Function), grace-period timer (D-20, needs G1 length). Push the built code, then test the two-app + QR flow with a real member account.
