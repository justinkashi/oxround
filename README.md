...
- **Phase:** LIVE (real auth in production). Supabase project `oxround` (ref `ymhbsvjqklqtfcitonzs`, ca-central-1): all 7 migrations applied, edge functions `check-in` + `auth-hook` + `invite-member` all ACTIVE, token hook + buckets + Resend SMTP configured. Magic-link login works end-to-end; the owner lands in the real CRM. The deployed site always runs against the live Supabase backend.
- **Built:** full clickable CRM (dashboard/live feed/at-risk, members, classes grid + rosters + waitlist, attendance, payments, plans, leads Kanban, coaches, announcements, reports+CSV, settings, login) + member-app preview at `/app` + responsive pass (mobile nav, stacking layouts, scrollable tables — desktop unchanged).
...
- **G1 FEEDBACK ROUND (2026-07-07, in progress — items 1–8/9 done):** Members page header stats — Total members + New this month cards (click = filter), new `getMemberStats()` in data.ts. Member profile fields/edit built on Fighter Card + Members edit modal: first/last/email/phone, DOB→age, weight class, date joined, membership type edit, and Timeline renamed Activity. Class location removed from the Classes create flow while keeping the DB column for compatibility. Class templates can now be edited (name, description, coach, days, time, duration, capacity, color); future sessions update, obsolete no-booking sessions cancel, and booked sessions stay intact. Classes and Schedule are now split: `/classes` manages templates, `/schedule` shows the weekly calendar with overlapping sessions side-by-side. Coaches can now be added directly, edited, promoted from existing members, and invited by email. Leads now auto-start trials with selectable length, show Day X of Y / expired state, and expired trials appear in the owner bell. Announcements now post quietly by default with an unchecked Notify members checkbox that creates notification rows when enabled. Remaining, one-by-one: settings website domain.
...
- **MEMBER APP IS ITS OWN FULL-SCREEN PRODUCT (2026-07-08):** `/app` no longer renders the CRM sidebar or the fake iPhone frame — it's a full-screen phone-width column (centered on desktop) via a new route-aware `app/shell.tsx`. Members log in and land straight in it; the "Member App Preview" link was removed from the CRM sidebar (staff no longer see the member app). Purely front-end layout — no Supabase/Vercel changes. Typechecked clean.
- **Last updated:** 2026-07-08 (**email+password login built** — all roles sign in with email+password on `/login` (primary), with "email me a link" + "forgot password?" fallbacks. First login after an invite routes to a skippable `/auth/set-password` screen; a `user_metadata.password_set` flag tracks completion so the nudge stops. Forgot-password recovery email also lands on `/auth/set-password`. No new tables/schema — passwords live in Supabase `auth.users`. Config note: add `/auth/set-password` to the Supabase Auth "Redirect URLs" allow-list so reset emails work. Typecheck clean). Prior: 2026-07-08 (member app full-screen shell — sidebar + phone frame removed from `/app`). Prior: 2026-07-07 AM (G1 feedback item 8 built and typechecked: announcements Post + optional Notify members notification rows). Prior: G1 feedback item 7 built and typechecked: leads trial automation. Prior: G1 feedback item 6 built and typechecked: coaches add/edit + invite. Prior: G1 feedback item 5 built and typechecked: Classes/Schedule split + overlap rendering. Prior: G1 feedback item 4 built and typechecked: class edit modal + future-session sync. Prior: G1 feedback item 3 built and typechecked: class location removed from Classes UI. Prior: G1 feedback item 2 built and typechecked: member profile fields/edit + Activity rename. Prior: bilingual EN/FR rollout built and typechecked. Prior: 2026-07-06 PM (VERSION 2 resilience + Twenty transfer session — see the two bullets above). Prior: 2026-07-05 (**CRITICAL FIX — migration 0007:** the `authenticated` role had NO table grants, so every logged-in read came back empty and every write failed with "permission denied" — this is exactly why **Add member** showed "could not add." Granted SELECT/INSERT/UPDATE to `authenticated` on all public tables (DELETE withheld per D-03); RLS still gates which rows. Verified live: the full add-member path succeeds under real owner login. Also this session: Members CSV bulk-import; **phone now optional** (blank → null); real DB error messages now surface in the form instead of a generic fallback; notification-bell dropdown no longer clips on desktop. State: production = real auth, 7 migrations applied. **invite-member redeployed v4 with `verify_jwt` OFF** (it self-authenticates via getUser + staff-role check) — this fixes the silent non-2xx that blocked invite emails (the gateway was rejecting the call before the body ran); it now also links already-registered users and logs failures. Add-member shows a clear ✓ and treats the invite as best-effort. Prior: login Member/Coach/Owner tabs are cosmetic — server routes by real role.)
